...
try:
    ...
    message = s.recv(1024)      # Messages received will be saved in variable message and byte is max 1024
    while (message):
        print("Message :",message.decode())     # Decoding the encoded message received
        message = s.recv(1024)                  # Loop keeps decoding and storing it into message variable.
